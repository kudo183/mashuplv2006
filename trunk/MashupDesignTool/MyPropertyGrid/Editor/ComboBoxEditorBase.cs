	...
	public abstract class ComboBoxEditorBase : ValueEditorBase
	{
		...
		void ShowComboBox()
		{
			...
			for (int i = 0; i < this.cbo.Items.Count; i++)
			{
				...
				if (CompareItemValue(currentValue, val) == true)
				{
        ...
        #region Virtual
        protected virtual bool CompareItemValue(object value, object itemValue)
        {
            return (value.Equals(itemValue) || value.ToString() == itemValue.ToString());
        }
		...
		{
            SetCurrentValue(e.AddedItems[0]);
